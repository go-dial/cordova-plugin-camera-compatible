...
# cordova-plugin-camera-compatible
...
## What Makes This Version "Compatible"

This is an enhanced version of the original Apache Cordova Camera Plugin with critical fixes for modern Android versions:

- **Fixed Gallery Selection Crash**: Resolved NullPointerException when selecting images from gallery on modern Android devices
- **Optimized Media Picker Support**: Improved handling of Android's optimized media picker to avoid permission issues
- **Better Error Handling**: Added comprehensive null checks and meaningful error messages
- **Scoped Storage Compatibility**: Enhanced compatibility with Android 10+ scoped storage requirements
- **Duplicate Code Cleanup**: Removed redundant code paths that could cause unexpected behavior

### Key Bug Fixes
- Fixed request code handling in `onActivityResult` method
- Added null checks for `imageUri` and `croppedFilePath` in `processResultFromCamera`
- Proper routing between optimized photo picker and legacy gallery access
- Cleaned up duplicate bitmap processing logic

For detailed information about the fixes, see [CRASH_FIX_SUMMARY.md](CRASH_FIX_SUMMARY.md).
...
The variable is configured on install time
    ...
    cordova plugin add cordova-plugin-camera-compatible --variable ANDROIDX_CORE_VERSION=1.8.0

## How to Contribute

Contributors are welcome! And we need your contributions to keep the project moving forward. You can report bugs, improve the documentation, or [contribute code](https://github.com/go-dial/cordova-plugin-camera-compatible/pulls).

**Have a solution?** Send a [Pull Request](https://github.com/go-dial/cordova-plugin-camera-compatible/pulls).

This is a compatible version of the original Apache Cordova Camera Plugin with additional fixes for modern Android versions.
